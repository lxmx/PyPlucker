...
TextParser.py   $Id: TextParser.py,v 1.75 2007/08/28 16:33:29 desrod Exp $
...
junk_alt_attributes = ("img", "spacer", "")
    ...
    def start_p (self, attributes):
        ...
        if 'align' in attribs:
            ...
            if align == 'left':
                ...
                # DRS 2004-12-29
                message("Unknown alignment '%s' requested in <p>" % align)
        if self.atable is not None and self.in_cell:
    ...
    def start_div (self, attributes):
        ...
        if 'align' in attributes:
            ...
            if align == 'left':
                ...
                # DRS 2004-12-29
                message("Unknown alignment '%s' requested in <div>" % align)
        if self.atable is not None and self.in_cell:
            self.atable.set_align (alignment)
        else:
            self._doc.add_vspace (0)
    ...
    def start_td (self, attributes):
        ...
        if 'align' in attributes:
            ...
            if align == 'left':
                ...
                # DRS 2004-12-29
                message("Unknown alignment '%s' requested in <td>/<th>" % align)
        self._doc.set_alignment (alignment)
        if self.atable is not None:
...
#################################################################
# Chris Hawks:
# Override function to handle anything marked <! ... >
# Python 2.2 and earlier only handles declarations.
# Python 2.3 and up handle more, but, we don't care about them.
#
# JimJ:
# Unless we want to start handling CDATA (or javascript or style)
# better ... so only override if the library version is old enough
# to need an override.
#################################################################
    try:
        import _markupbase
        ...
        # other types are also "handled" by this workaround.
        _MARKEDSECTIONCLOSE = re.compile(r']\s*>')

        def parse_declaration(self, i):
            rawdata = self.rawdata
            assert rawdata[i:i+2] == "<!", "unexpected call to parse_declaration"
            if rawdata[i+2:i+3] in ("-", ""):
                # Start of comment followed by buffer boundary,
                # or just a buffer boundary.
                return -1
            if rawdata[i+2:i+3] == '--': #comment
                look = _COMMENTCLOSE
            elif rawdata[i+2] == '[': #marked section
                look = _MARKEDSECTIONCLOSE
            else:
                look = re.compile(r'>')
            match = look.search(rawdata, i+2)
            if not match:
                return -1
            return match.end(0)
