...
"""
Spider.py   $Id: Spider.py,v 1.101 2005/04/04 01:46:07 chrish Exp $
...
class SpiderLink:
    ...
    def set_stay_on_domain (self, host):
        self._stay_on_domain = get_domain_from_host(host)
    ...
    def check_fetch (self, as_image):
        ...
        if self._stay_on_domain is not None:
            if self._stay_on_domain != get_domain_from_host(self._url.get_host()):
...
class Spider:
    ...
    def __init__ (self, retriever,
        ...
        # _robot_list contains a list of domains who's robots.txt info
        # for 'user_agent' has been added to self._exclusion_list
        self._robot_list = []
    ...
    def _check_robot(self, url):
        """Check url's server for exclusions by robots.txt"""
        this_url = URL(url)
        this_domain = get_domain_from_host(this_url.get_host())
        if this_domain not in self._robot_list:
            self._robot_list.append(this_domain)
            mine = []
            default = []
            mine_all = 0
            default_all = 0
            user_agent = self._config.get_string('user_agent', \
                                                 'Plucker/Py-%s' % __version__)
                ...
                for line in lines:
                    ...
                    if line[0] == "user-agent":
                        if line[1] == '*':    # Everyone
                            ...
                            name = line[1].replace('*', '').lower()
                            if re.search(name, user_agent, re.IGNORECASE):
                          	     state = 1000001   # Me

                    if line[0] == "disallow" and state:
                        # High numbers ensure first check by Exclusion.py
                        location = line[1]

                        if not location:      # empty disallow means allow all
                            if state == 1000001:
                                mine_all = 1
                            elif state == 1000000:
                                default_all = 1
                        ...
                        entry = '%d:-:%s' % (state, location)
                        if state == 1000001:
                            mine.append(entry)
                        else:
                            default.append(entry)

            if mine_all:                  # We have full access
                list_all = 1              # Skip processing

            elif len(mine):               # We are in file, process our entries
                list = mine
                list_all = 0

            else:                         # Process defaults
                list = default
                list_all = default_all

            if not list_all:              # Process
                if len(list):
                    for entry in list:
                        if entry[10:-3] == homepath:
                            error("\'%s\' will not allow \'%s\' access.\n" \
                                  % (robotpath, user_agent))
                            sys.exit(2)

                        message('Add %s to Exclusions (from robots.txt)' %entry)
...
def main (config, excl_lists=[]):
    ...
    if socket_timeout:
        try:
            # Timeout was added to the standard library in python 2.3
            import socket
            socket.setdefaulttimeout(socket_timeout)
            message(2, "Setting (socket) retrieval timeout to %d seconds",
                    socket_timeout)
        except ImportError:
            # Don't crash -- it *could* be a cache-only or help-only run.
            message(2, "No socket?!?")
        except AttributeError:
            # If the socket module doesn't have that function, use our
            # workaround.
            try:
            ...
            message(2,
                    "Setting (timeoutsocket) retrieval timeout to %d seconds",
                    socket_timeout)
        except:
            # Actually, we might have already set it before the error -- but
            # probably not.
            message(2, "Couldn't set retrieval timeout to desired %d seconds"
                    " -- will wait forever if necessary", socket_timeout)
            # In case of KeyboardInterrupt, etc.
            raise
...
def realmain (outputstream, argv=None):
    ...
    def display_help (pluckerhome=pluckerhome):
        ...
        message(0, "    --no-image-alt:")
        message(0, "                   Don't show alternate text for images.")
        message(0, "")
    ...
    try:
        ...
        (opts, args) = getopt.getopt(argv[1:], "f:chqvV:p:P:H:E:M:N:s:", \
                                     [  "db-file=", "doc-file=", "help",
                                        ...
                                        "fragments=", "creator-id=", "filter=",
                                        "bookmarks=", "no-image-alt"])
        if args:
    ...
    if no_image_alt is not None:
        config.set ('no_image_alt', 1)
